...
function dx = quadrotor3D_dynamics(t, x, p)
    ...
    % tau_psi = u4)
    u2 = Kp_phi * (phi_des - phi) + Kd_phi * (0 - phi_dot);
    u3 = Kp_theta * (theta_des - theta) + Kd_theta * (0 - theta_dot);
    u4 = Kp_psi * (psi_des - psi) + Kd_psi * (0 - psi_dot);
