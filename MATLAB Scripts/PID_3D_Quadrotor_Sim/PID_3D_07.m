...
% Trajectory - constant speed circle with increasing radius
% Control - PID
...
params.x_des = @(t) (1+0.2*t)*cos(0.3*t);
params.y_des = @(t) (1+0.2*t)*sin(0.3*t);
params.z_des = @(t) 2;

params.x_dot_des = @(t) 0.2*cos(0.3*t) - 0.3*(1 + 0.2*t).*sin(0.3*t);
params.y_dot_des = @(t) 0.2*sin(0.3*t) + 0.3*(1 + 0.2*t).*cos(0.3*t);
params.z_dot_des = @(t) 0;

params.x_ddot_des = @(t) -0.12*sin(0.3*t) - 0.09*(1 + 0.2*t).*cos(0.3*t);
params.y_ddot_des = @(t)0.12*cos(0.3*t) - 0.09*(1 + 0.2*t).*sin(0.3*t);
