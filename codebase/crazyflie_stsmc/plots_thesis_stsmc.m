% =========================================================================
% plots_thesis_stsmc.m — Thesis-Quality Simulation Plots (STSMC Controller)
% =========================================================================
% Generates and saves all 7 required figures:
%   Fig 1 : 3D Trajectory
...
%   Fig 7 : Sliding Variables      (s_phi, s_theta, s_psi, s_x, s_y, s_z)
%
% INPUTS:
%   t    - time vector  (N-1 x 1)
%   x    - state matrix (N-1 x 12)  in STANDARD order:
%          [x y z phi theta psi x_dot y_dot z_dot phi_dot theta_dot psi_dot]
%          (reordered from STSMC convention inside stsmc_main.m)
%   logs - signals struct from stsmc_main.m
%   p    - params stub (p.sat_bl used only for identification)
%
% SAVED FILES (300 dpi PNG):
%   STSMC_Fig1_3D_Trajectory.png
%   STSMC_Fig2_Position_Tracking.png
%   STSMC_Fig3_Attitude_Tracking.png
%   STSMC_Fig4_Position_Errors.png
%   STSMC_Fig5_Attitude_Errors.png
%   STSMC_Fig6_Control_Inputs.png
%   STSMC_Fig7_Sliding_Variables.png
% =========================================================================
...
function plots_thesis_stsmc(t, x, logs, p)
    ...
    plot3(x(1,1),   x(1,2),   x(1,3),   'ko', 'MarkerSize', 8, ...
          'MarkerFaceColor', 'k', 'DisplayName', 'Start');
    plot3(x(end,1), x(end,2), x(end,3), 'ks', 'MarkerSize', 8, ...
    ...
    % Use commanded attitude (phi_cmd, theta_cmd) as reference — shows
    % inner-loop tracking quality.  phi_cmd/theta_cmd logged from outer loop.
    % ======================================================================
    fig3 = figure('Color', 'w', 'Position', [100 100 fig_w fig_h]);

    % Use commanded attitude as reference if available; else fall back to zeros
    if isfield(logs, 'phi_cmd')
        phi_ref_plt   = rad2deg(logs.phi_cmd);
        theta_ref_plt = rad2deg(logs.theta_cmd);
    else
        phi_ref_plt   = zeros(size(t));
        theta_ref_plt = zeros(size(t));
    end
    psi_ref_plt = rad2deg(logs.psi_ref);

    subplot(3,1,1)
    plot(t, phi_ref_plt, '--', 'Color', colors.refclr, 'LineWidth', line_w, 'DisplayName', '\phi_{cmd}');
    hold on;
    ...
    subplot(3,1,2)
    plot(t, theta_ref_plt, '--', 'Color', colors.refclr, 'LineWidth', line_w, 'DisplayName', '\theta_{cmd}');
    hold on;
    ...
    subplot(3,1,3)
    plot(t, psi_ref_plt, '--', 'Color', colors.refclr, 'LineWidth', line_w, 'DisplayName', '\psi_{ref}');
    hold on;
    ...
    subplot(4,1,1)
    plot(t, logs.U1_cmd_log, '-', 'Color', colors.blue, 'LineWidth', line_w);
    ylabel('U_1 (N)', 'FontSize', font_size);
    title('Total Thrust', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    subplot(4,1,2)
    plot(t, logs.U2_cmd_log, '-', 'Color', colors.green, 'LineWidth', line_w);
    ylabel('U_2 (N·m)', 'FontSize', font_size);
    title('Roll Torque', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    subplot(4,1,3)
    plot(t, logs.U3_cmd_log, '-', 'Color', colors.orange, 'LineWidth', line_w);
    ylabel('U_3 (N·m)', 'FontSize', font_size);
    title('Pitch Torque', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    subplot(4,1,4)
    plot(t, logs.U4_cmd_log, '-', 'Color', colors.purple, 'LineWidth', line_w);
    ylabel('U_4 (N·m)', 'FontSize', font_size);
    xlabel('Time (s)', 'FontSize', font_size);
    ...
    % Left column : attitude surfaces  (s_phi, s_theta, s_psi)
    % Right column : position surfaces (s_x, s_y, s_z)
    % Layout matches SMC Fig 7 exactly.
    % ======================================================================
    fig7 = figure('Color', 'w', 'Position', [100 100 fig_w fig_h+100]);

    subplot(3,2,1)
    plot(t, logs.s_phi, '-', 'Color', colors.blue, 'LineWidth', line_w);
    yline(0, 'k--', 'LineWidth', 0.8);
    ylabel('s_\phi', 'FontSize', font_size);
    title('Roll', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    subplot(3,2,3)
    plot(t, logs.s_theta, '-', 'Color', colors.green, 'LineWidth', line_w);
    yline(0, 'k--', 'LineWidth', 0.8);
    ylabel('s_\theta', 'FontSize', font_size);
    title('Pitch', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    subplot(3,2,5)
    plot(t, logs.s_psi, '-', 'Color', colors.orange, 'LineWidth', line_w);
    yline(0, 'k--', 'LineWidth', 0.8);
    ylabel('s_\psi', 'FontSize', font_size);
    xlabel('Time (s)', 'FontSize', font_size);
    title('Yaw', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    subplot(3,2,2)
    plot(t, logs.s_x, '-', 'Color', colors.blue, 'LineWidth', line_w);
    yline(0, 'k--', 'LineWidth', 0.8);
    ylabel('s_x', 'FontSize', font_size);
    title('X Position', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    subplot(3,2,4)
    plot(t, logs.s_y, '-', 'Color', colors.green, 'LineWidth', line_w);
    yline(0, 'k--', 'LineWidth', 0.8);
    ylabel('s_y', 'FontSize', font_size);
    title('Y Position', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    subplot(3,2,6)
    plot(t, logs.s_z, '-', 'Color', colors.orange, 'LineWidth', line_w);
    yline(0, 'k--', 'LineWidth', 0.8);
    ylabel('s_z', 'FontSize', font_size);
    xlabel('Time (s)', 'FontSize', font_size);
    title('Altitude', 'FontSize', font_size);
    grid on; set(gca, 'FontSize', font_size);

    sgtitle([ctrl_name ' — Sliding Variables'], 'FontSize', font_size+1);
    save_fig(fig7, [ctrl_name '_Fig7_Sliding_Variables.png']);
    fprintf('[Saved] %s_Fig7_Sliding_Variables.png\n', ctrl_name);

    fprintf('\n[plots_thesis_stsmc] All figures saved to: %s\n', save_dir);
