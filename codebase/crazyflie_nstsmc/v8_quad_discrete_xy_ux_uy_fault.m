% v7 — merged NS-STSMC
% uses v4 control law when |e_dot| >= tol, v6 control law when |e_dot| < tol
% tol = 1e-6 guards against ephidot^(1-gamma) singularity (exponent = -2/3) in all channels
...
beta_z = 5; % previously all beta were one
beta_phi = 2;
beta_theta = 2;
beta_psi = 1;
beta_x = 2;
...
tol = 1e-6;   % threshold for near-zero error derivative (guards ė^(1-gamma) singularity)
...
for i = 1 : N-1
    ...
    if abs(ezdot) < tol
        ...
        % v4 form — standard NS-STSMC
        u1_cmd = (m / (cos(x_k(1)) * cos(x_k(2)))) * (zcmd_ddot(i) + g + beta_z^gamma*(1/gamma)*abs(ezdot^(2-gamma)) * sign(ezdot) + beta_z^gamma*(1/gamma)*abs(ezdot^(1-gamma)) * sign(ezdot) * (k1_z * sqrt(abs(sz)) * sign(sz) + k2_z * I_u1));
    end
    ...
    if abs(exdot) < tol
        ...
        % v4 form
        uxdash = m * (xd_ddot + beta_x^gamma * (1/gamma) * abs(exdot^(2-gamma)) * sign(exdot) + beta_x^gamma * (1/gamma) * abs(exdot^(1-gamma))* sign(exdot) * (k1_x * sqrt(abs(sx)) * sign(sx) + k2_x * I_ux));
    end
    ...
    if abs(eydot) < tol
        ...
        % v4 form
        uydash = m * (yd_ddot + beta_y^gamma * (1/gamma) * abs(eydot^(2-gamma))* sign(eydot) + beta_y^gamma * (1/gamma) * abs(eydot^(1-gamma))* sign(eydot) * (k1_y * sqrt(abs(sy)) * sign(sy) + k2_y * I_uy));
    end
    ...
    if abs(ephidot) < tol
        ...
        % v4 form
        u2_cmd = Jx * (phicmd_ddot(i) - ((Jy-Jz)/Jx)*x_k(5)*x_k(6) + beta_phi^gamma * (1/gamma) * abs(ephidot^(2-gamma))*sign(ephidot) + beta_phi^gamma * (1/gamma) * abs(ephidot^(1-gamma))*sign(ephidot) * (k1_att * sqrt(abs(sphi)) * sign(sphi) + k2_att * I_u2));
    end
    ...
    if abs(ethetadot) < tol
        ...
        % v4 form
        u3_cmd = Jy * (thetacmd_ddot(i) - ((Jz-Jx)/Jy)*x_k(4)*x_k(6) + beta_theta^gamma * (1/gamma) * abs(ethetadot^(2-gamma))*sign(ethetadot) + beta_theta^gamma * (1/gamma) * abs(ethetadot^(1-gamma))*sign(ethetadot) * (k1_att * sqrt(abs(stheta)) * sign(stheta) + k2_att * I_u3));
    end
    ...
    if abs(epsidot) < tol
        ...
        % v4 form
        u4_cmd = Jz * (psicmd_ddot(i) - ((Jx-Jy)/Jz)*x_k(4)*x_k(5) + beta_psi^gamma * (1/gamma) * abs(epsidot^(2-gamma))*sign(epsidot) + beta_psi^gamma * (1/gamma) * abs(epsidot^(1-gamma))*sign(epsidot) * (k1_psi * sqrt(abs(spsi)) * sign(spsi) + k2_psi * I_u4));
    end
