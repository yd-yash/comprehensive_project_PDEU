...
Jx = 16.571710e-6;   % kg·m²
Jy = 16.655602e-6;   % kg·m²
Jz = 29.261652e-6;   % kg·m²
m  = 0.034;          % kg
g  = 9.81;
l  = 0.046;          % m
b  = 2.88e-8;        % N·s²/rad²
d  = 7.24e-10;       % N·m·s²/rad²
T_min = 0;
T_max = 0.16;        % N per rotor (total max = 0.64 N)

% NTSM parameters
p = 5; q = 3;
gamma = p/q;

beta_z     = 3;   % unchanged — altitude channel, no normalise() wrapping
beta_phi   = 4;   % CF: flip_zone=38.3deg > 2*phi_max=24deg  (14.3deg margin)
beta_theta = 4;   % same as phi
beta_psi   = 1;   % unchanged — yaw, flip_zone=386deg >> 24deg
beta_x     = 2;   % unchanged — position channels have no normalise() wrapping
beta_y     = 1;

tau_att_max = 0.5e-3;      % 0.35 mNm — roll/pitch physical torque limit
tau_psi_max = 0.15e-3;      % 0.15 mNm — yaw physical torque limit
sin_max     = sin(deg2rad(12)); % max |ux|,|uy|: keeps 2*phi_max<flip_zone
u1_max      = 4 * T_max;    % 0.64 N total thrust ceiling
...
alpha_f_phi   = 0.1;   % phi filter — already working well
...
% k1_x = 0.1;   k2_x = 0.35;
% k1_y = 0.1;   k2_y = 0.35;
k1_z   = 4.0;    k2_z   = 0.20;    % z settles in ~20s vs 120s previously
k1_att = 30;     k2_att = 0.8;     % stronger attitude correction
k1_psi = 10;     k2_psi = 0.05;    % unchanged
k1_x   = 1.2;    k2_x   = 0.08;
k1_y   = 1.2;    k2_y   = 0.08;
...
for i = 1 : N-1
    ...
    % xd(i) = 2*cos(0.1*t(i));
    % yd(i) = 2*sin(0.2*t(i));
    zcmd(i)       =  1.0 + 0.2 * sin(0.5 * t(i));
    zcmd_dot(i)   =  0.2 * 0.5 * cos(0.5 * t(i));
    zcmd_ddot(i)  = -0.2 * 0.25 * sin(0.5 * t(i));
    
    psicmd(i)      = 0;
    psicmd_dot(i)  = 0;
    psicmd_ddot(i) = 0;
    
    xd(i) = 0.5 * sin(0.5 * t(i));
    yd(i) = 0.5 * sin(1.0 * t(i)); 
    ...
    u1_cmd = max(0.001, min(u1_max, u1_cmd));
    ...
    % xy - vel, acc references  (replace the existing 4 lines)
    xd_dot  = 0.5*0.5*cos(0.5*t(i))  + kpx*(xd(i) - x_k(7));
    xd_ddot = -0.5*0.25*sin(0.5*t(i)) + kdx*(xd_dot - x_k(10));
    yd_dot  = 0.5*1.0*cos(1.0*t(i)) + kpy*(yd(i) - x_k(8));   % add feedforward
    yd_ddot = -0.5*1.0^2*sin(1.0*t(i)) + kdy*(yd_dot - x_k(11));
    ...
    uy = f_sat(uydash / max(abs(u1_cmd), m*g*0.3), -sin_max, sin_max);
    u_y(i) = uy;
    ...
    thetacmd_ddot(i) = max(-50, min(50, thetacmd_ddot_filt));  % rad/s² clamp
