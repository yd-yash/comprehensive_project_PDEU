...
mu_min = 1e-3;   % singularity protection
...
for i = 1 : N-1
    ...
    % STA term
    vz = -k1*sqrt(abs(sz))*sign(sz) - k2*I_u1;
    
    % mu
    mu_z = (p/(q*beta_z^gamma)) * abs(ezdot)^(gamma-1);
    mu_z = max(mu_z, mu_min);
    
    % required error acceleration
    ezddot_req = (vz - ezdot) / mu_z;
    ...
    vphi = -k1*sqrt(abs(sphi))*sign(sphi) - k2*I_u2;
    
    mu_phi = (p/(q*beta_phi^gamma))*abs(ephidot)^(gamma-1);
    mu_phi = max(mu_phi, mu_min);
    
    ephiddot_req = (vphi - ephidot)/mu_phi;
    ...
    vtheta = -k1*sqrt(abs(stheta))*sign(stheta) - k2*I_u3;
    
    mu_theta = (p/(q*beta_theta^gamma))*abs(ethetadot)^(gamma-1);
    mu_theta = max(mu_theta, mu_min);
    
    ethetaddot_req = (vtheta - ethetadot)/mu_theta;
    ...
    vpsi = -k1*sqrt(abs(spsi))*sign(spsi) - k2*I_u4;
    
    mu_psi = (p/(q*beta_psi^gamma))*abs(epsidot)^(gamma-1);
    mu_psi = max(mu_psi, mu_min);
    
    epsiddot_req = (vpsi - epsidot)/mu_psi;
