...
for i = 1 : N-1
    ...
    u1_cmd = (m / (cos(x_k(1)) * cos(x_k(2)))) * (zcmd_ddot(i) + g + beta_z^gamma*(1/gamma)*ezdot^(2-gamma) + k1_z * sqrt(abs(sz)) * sign(sz) + k2_z * I_u1);
    ...
    uxdash = m * (xd_ddot + beta_x^gamma * (1/gamma) * exdot^(2-gamma) + k1_x * sqrt(abs(sx)) * sign(sx) + k2_x * I_ux);
    ...
    uydash = m * (yd_ddot + beta_y^gamma * (1/gamma) * eydot^(2-gamma) + k1_y * sqrt(abs(sy)) * sign(sy) + k2_y * I_uy);
    ...
    u2_cmd = Jx * (phicmd_ddot(i) - ((Jy-Jz)/Jx)*x_k(5)*x_k(6) + beta_phi^gamma * (1/gamma) * ephidot^(2-gamma) + k1_att * sqrt(abs(sphi)) * sign(sphi) + k2_att * I_u2);
    ...
    u3_cmd = Jy * (thetacmd_ddot(i) - ((Jz-Jx)/Jy)*x_k(4)*x_k(6) + beta_theta^gamma * (1/gamma) * ethetadot^(2-gamma) + k1_att * sqrt(abs(stheta)) * sign(stheta) + k2_att * I_u3);
    ...
    u4_cmd = Jz * (psicmd_ddot(i) - ((Jx-Jy)/Jz)*x_k(4)*x_k(5) + beta_psi^gamma * (1/gamma) * epsidot^(2-gamma) + k1_psi * sqrt(abs(spsi)) * sign(spsi) + k2_psi * I_u4);
