...
for i = 1 : N-1
    ...
    u1_cmd = (m / (cos(x_k(1)) * cos(x_k(2)))) * (zcmd_ddot(i) + g + beta_z^gamma*(1/gamma)*ezdot^(2-gamma) + beta_z^gamma*(1/gamma)*ezdot^(1-gamma) * (k1_z * sqrt(abs(sz)) * sign(sz) + k2_z * I_u1));
    % vz = -k1_z*sqrt(abs(sz))*sign(sz) - k2_z*I_u1;
    % 
    % mu_z = (p/(q*beta_z^gamma)) * abs(ezdot)^(gamma-1);
    % mu_z = max(mu_z, mu_min);
    % 
    % ezddot_req = (vz - ezdot) / mu_z; %* mu_z/(mu_z^2 + 0.01); 
    % 
    % 
    % u1_cmd = (m/(cos(x_k(1))*cos(x_k(2)))) * ...
    %          (zcmd_ddot(i) + g - ezddot_req);
    ...
    % vx = -k1_x*sqrt(abs(sx))*sign(sx) - k2_x*I_ux;
    % 
    % mu_x = (p/(q*beta_x^gamma)) * abs(exdot)^(gamma-1);
    % mu_x = max(mu_x, mu_min); 
    % 
    % exddot_req = (vx - exdot) / mu_x; % * mu_x/(mu_x^2 + 0.01); 
    % 
    % uxdash = m * (xd_ddot - exddot_req); 
    % ux = f_sat(uxdash / max(u1_cmd, 1), -0.5, 0.5);
    uxdash = m * (xd_ddot + beta_x^gamma * (1/gamma) * exdot^(2-gamma) + beta_x^gamma * (1/gamma) * exdot^(1-gamma) * (k1_x * sqrt(abs(sx)) * sign(sx) + k2_x * I_ux));
    ...
    % vy = -k1_y*sqrt(abs(sy))*sign(sy) - k2_y*I_uy;
    % 
    % mu_y = (p/(q*beta_y^gamma)) * abs(eydot)^(gamma-1);
    % mu_y = max(mu_y, mu_min);
    % 
    % eyddot_req = (vy - eydot) / mu_y; %* mu_y/(mu_y^2 + 0.01); 
    % 
    % uydash = m * (yd_ddot - eyddot_req);
    uydash = m * (yd_ddot + beta_y^gamma * (1/gamma) * eydot^(2-gamma) + beta_y^gamma * (1/gamma) * eydot^(1-gamma) * (k1_y * sqrt(abs(sy)) * sign(sy) + k2_y * I_uy));
    ...
    % vphi = -k1_att*sqrt(abs(sphi))*sign(sphi) - k2_att*I_u2;
    % 
    % mu_phi = (p/(q*beta_phi^gamma))*abs(ephidot)^(gamma-1);
    % mu_phi = max(mu_phi, mu_min);
    % 
    % ephiddot_req = (vphi - ephidot) / mu_phi; %* mu_phi/(mu_phi^2 + 0.01); 
    % 
    % u2_cmd = Jx*(phicmd_ddot(i) ...
    %             - ((Jy-Jz)/Jx)*x_k(5)*x_k(6) ...
    %             - ephiddot_req);

    u2_cmd = Jx * (phicmd_ddot(i) - ((Jy-Jz)/Jx)*x_k(5)*x_k(6) + beta_phi^gamma * (1/gamma) * ephidot^(2-gamma) + beta_phi^gamma * (1/gamma) * ephidot^(1-gamma) * (k1_att * sqrt(abs(sphi)) * sign(sphi) + k2_att * I_u2));
    ...
    % vtheta = -k1_att*sqrt(abs(stheta))*sign(stheta) - k2_att*I_u3;
    % 
    % mu_theta = (p/(q*beta_theta^gamma))*abs(ethetadot)^(gamma-1);
    % mu_theta = max(mu_theta, mu_min);
    % 
    % ethetaddot_req = (vtheta - ethetadot) / mu_theta; %* mu_theta/(mu_theta^2 + 0.01); 
    % 
    % u3_cmd = Jy*(thetacmd_ddot(i) ...
    %             - ((Jz-Jx)/Jy)*x_k(4)*x_k(6) ...
    %             - ethetaddot_req);
    u3_cmd = Jy * (thetacmd_ddot(i) - ((Jz-Jx)/Jy)*x_k(4)*x_k(6) + beta_theta^gamma * (1/gamma) * ethetadot^(2-gamma) + beta_theta^gamma * (1/gamma) * ethetadot^(1-gamma) * (k1_att * sqrt(abs(stheta)) * sign(stheta) + k2_att * I_u3));
    ...
    % vpsi = -k1_psi*sqrt(abs(spsi))*sign(spsi) - k2_psi*I_u4;
    
    % mu_psi = (p/(q*beta_psi^gamma))*abs(epsidot)^(gamma-1);
    % mu_psi = max(mu_psi, mu_min);
    % 
    % epsiddot_req = (vpsi - epsidot) / mu_psi; %*mu_psi/(mu_psi^2 + 0.01); 
    % 
    % u4_cmd = Jz*(psicmd_ddot(i) ...
    %             - ((Jx-Jy)/Jz)*x_k(4)*x_k(5) ...
    %             - epsiddot_req);
    u4_cmd = Jz * (psicmd_ddot(i) - ((Jx-Jy)/Jz)*x_k(4)*x_k(5) + beta_psi^gamma * (1/gamma) * epsidot^(2-gamma) + beta_psi^gamma * (1/gamma) * epsidot^(1-gamma) * (k1_psi * sqrt(abs(spsi)) * sign(spsi) + k2_psi * I_u4));
